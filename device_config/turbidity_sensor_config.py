...
#
# Module: Turbidity Sensor ID Configuration V0.2
# Description: Interactive script to set the Turbidity Sensor device ID
# -----------------------------------------------------------------------------
...
def test_communication(ser, device_id):
    ...
    function_code = 0x03
    register_address = 0x0001  # Turbidity value register
...
def write_device_id(old_device_id, new_device_id, port='/dev/ttyS0'):
    """Change the device ID of the Turbidity sensor"""
    ser = serial.Serial(
        port=port,
        baudrate=9600,
    ...
    try:
        ...
        register_address = 0x0019  # Device ID register for Turbidity sensor
...
def main():
    ...
    print("\n=== Trübungssensor Adresskonfiguration ===\n")
    ...
    try:
        ...
        logging.error(f"Fehler beim Konfigurieren des Trübungssensors: {e}")
